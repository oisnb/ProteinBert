...
from scikeras.wrappers import \
    KerasClassifier  ##from tensorflow.keras.wrappers.scikit_learn import KerasClassifier, KerasRegressor已被移除 用scikeras
import pickle
...
def finetune(model_generator, input_encoder, output_spec, train_seqs, train_raw_Y, valid_seqs=None, valid_raw_Y=None,
             seq_len=512, batch_size=32, \
             max_epochs_per_stage=40, lr=None, begin_with_frozen_pretrained_layers=True,
             lr_with_frozen_pretrained_layers=None, n_final_epochs=1, \
             final_seq_len=1024, final_lr=None, callbacks=[]):
    ###先训练最后一层 可能训练40也可能更少  训练完最后一层后再训练整个网络
    # 训练集 验证集编码
    encoded_train_set, encoded_valid_set = encode_train_and_valid_sets(train_seqs, train_raw_Y, valid_seqs, valid_raw_Y,
    ...
    if begin_with_frozen_pretrained_layers:  # 先训练最后一层
        ...
        _ = model_generator.train(encoded_train_set, encoded_valid_set, seq_len, batch_size, max_epochs_per_stage,
                                  lr=lr_with_frozen_pretrained_layers, \
                                  callbacks=callbacks, freeze_pretrained_layers=True)

    # 训练整个网络
    log('Training the entire fine-tuned model...')
    train_loss, valid_loss = model_generator.train(encoded_train_set, encoded_valid_set, seq_len, batch_size,
                                                   max_epochs_per_stage, lr=lr, callbacks=callbacks, \
    ...
    if n_final_epochs > 0:  # 按照特定seq长度训练整个网络
        ...
        encoded_train_set, encoded_valid_set = encode_train_and_valid_sets(train_seqs, train_raw_Y, valid_seqs,
                                                                           valid_raw_Y, input_encoder, output_spec,
                                                                           final_seq_len)
        _ = model_generator.train(encoded_train_set, encoded_valid_set, final_seq_len, final_batch_size, n_final_epochs,
                                  lr=final_lr, callbacks=callbacks, \
                                  freeze_pretrained_layers=False)
...
def evaluate_by_len(model_generator, input_encoder, output_spec, seqs, raw_Y, start_seq_len=512, start_batch_size=32,
    ...
    for len_matching_dataset, seq_len, batch_size in split_dataset_by_len(dataset, start_seq_len=start_seq_len,
        ...
        results.append(get_evaluation_results(y_true, y_pred, output_spec))  # 为什么下面要相同的操作
...
def get_evaluation_results(y_true, y_pred, output_spec, return_confusion_matrix=False):
    ...
    if output_spec.output_type.is_numeric:
        ...
        confusion_matrix = pd.DataFrame(
            confusion_matrix(y_true, y_pred_classes, labels=np.arange(output_spec.n_unique_labels)),
            index=str_unique_labels, \
...
def encode_train_and_valid_sets(train_seqs, train_raw_Y, valid_seqs, valid_raw_Y, input_encoder, output_spec, seq_len):
    encoded_train_set = encode_dataset(train_seqs, train_raw_Y, input_encoder, output_spec, seq_len=seq_len,
    ...
    if valid_seqs is None and valid_raw_Y is None:
        ...
        encoded_valid_set = encode_dataset(valid_seqs, valid_raw_Y, input_encoder, output_spec, seq_len=seq_len,
                                           needs_filtering=True, \
...
def encode_dataset(seqs, raw_Y, input_encoder, output_spec, seq_len=512, needs_filtering=True, dataset_name='Dataset',
                   verbose=True):
    if needs_filtering:
        dataset = pd.DataFrame({'seq': seqs, 'raw_Y': raw_Y})
...
def filter_dataset_by_len(dataset, seq_len=512, seq_col_name='seq', dataset_name='Dataset', verbose=True):
    ...
    if verbose:
        log('%s: Filtered out %d of %d (%.1f%%) records of lengths exceeding %d.' % (
        dataset_name, n_removed_records, len(dataset), 100 * n_removed_records / len(dataset), \
...
def predict_encode(seqs, input_encoder, seq_len=512, needs_filtering=True, dataset_name='Dataset', verbose=True):
    if needs_filtering:
...
def predict_our(model_generator, input_encoder, output_spec, seqs, start_seq_len=512, start_batch_size=32,
                increase_factor=2, threshold=0.5):
    assert model_generator.optimizer_weights is None
    ...
    for len_matching_dataset, seq_len, batch_size in split_dataset_by_len(dataset, start_seq_len=start_seq_len,
        ...
        X = predict_encode(len_matching_dataset['seq'], input_encoder, \
                           seq_len=seq_len, needs_filtering=False)
