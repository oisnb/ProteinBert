...
class ModelGenerator:
    ...
    def __init__(self, optimizer_class=keras.optimizers.Adam, lr=2e-04, other_optimizer_kwargs={}, model_weights=None,
    ...
    def train(self, encoded_train_set, encoded_valid_set, seq_len, batch_size, n_epochs, lr=None, callbacks=[],
        ...
        history = model.fit(train_X, train_Y, sample_weight=train_sample_weigths, batch_size=batch_size,
                            epochs=n_epochs, validation_data=encoded_valid_set, \
                            callbacks=callbacks)
...
class PretrainingModelGenerator(ModelGenerator):
    ...
    def __init__(self, create_model_function, n_annotations, create_model_kwargs={},
                 optimizer_class=keras.optimizers.Adam, lr=2e-04, other_optimizer_kwargs={}, \
...
class FinetuningModelGenerator(ModelGenerator):
    ...
    def create_model(self, seq_len, freeze_pretrained_layers=False):
        ...
        model = self.pretraining_model_generator.create_model(seq_len, compile=False, init_weights=(
                    self.model_weights is None))  # 测试的时候self.model_weights不为0 训练时候为0吗
        ...
        if self.output_spec.output_type.is_categorical:
            output_layer = keras.layers.Dense(len(self.output_spec.unique_labels), activation='softmax')(
                last_hidden_layer)
        ...
        model.compile(loss=loss, optimizer=self.optimizer_class(learning_rate=self.lr,
                                                                **self.other_optimizer_kwargs))  # 无论训练还是测试都会走到这里 编译模型 训练就fit 测试就predict
        # 通过加metrics='accuracy'?
        self._init_weights(model)
...
def load_pretrained_model_from_dump(dump_file_path, create_model_function, create_model_kwargs={},
    ...
    model_generator = PretrainingModelGenerator(create_model_function, n_annotations,
                                                ...
                                                annots_loss_weight=annots_loss_weight, model_weights=model_weights,
                                                optimizer_weights=optimizer_weights)
...
def tokenize_seqs(seqs, seq_len):
    ...
    return np.array([seq_tokens + (seq_len - len(seq_tokens)) * [additional_token_to_index['<PAD>']] for seq_tokens in
                     map(tokenize_seq, seqs)], dtype=np.int32)
